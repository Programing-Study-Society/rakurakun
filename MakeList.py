...
def makeList(programPaths):
    ...
    EndButton = tk.Button(makeList_win, text='選択完了',command=lambda:make_plot(makeList_win,list_plot_keyword,programPaths,list_chk))
    EndButton.grid(row=erow,column=3)   #3列目
    # #完了が押されずにタブが閉じられた時の処理
    # makeList_win.protocol("WM_DELETE_WINDOW", command = lambda:click_close())

    #全てにチェックを入れる
    allSelect_click(list_chk)

    #完了が押されるまでウィンドウを維持
    makeList_win.wait_window(makeList_win)

    #閉じられたとき、チェックが入っている場所(True,Falseの配列)を返す
...
def make_plot(window, list_plot_keyword, list_keyword, list_chk):
    ...
    for ilist in range(num_list):
        ...
        if bln == True:  #チェック済みの行
            list_plot_keyword.append(True)
        else:
            list_plot_keyword.append(False)
